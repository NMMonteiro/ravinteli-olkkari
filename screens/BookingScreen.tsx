...
const BookingScreen: React.FC = () => {
  ...
  const [date, setDate] = useState('2023-11-05'); // Default matching the mock UI
  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark text-white font-display pb-24">
      {/* Top Bar */}
      <div className="sticky top-0 z-20 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md px-4 py-4 flex items-center justify-between border-b border-primary/20">
        <div onClick={() => navigate(-1)} className="flex size-10 items-center justify-start cursor-pointer">
          <span className="material-symbols-outlined text-white">arrow_back_ios</span>
        </div>
        <h2 className="text-lg font-bold leading-tight tracking-tight text-white flex-1 text-center font-display uppercase tracking-widest">Book a Table</h2>
        <div className="size-10 flex items-center justify-end">
          <span className="material-symbols-outlined text-white cursor-pointer">info</span>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        {/* Date Selection (Simplified for MVP) */}
        <div className="px-4">
