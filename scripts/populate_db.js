...
const menuItems = [
    ...
    { name: 'Salmon', description: 'Smoked grilled salmon, capers beurre blanc, garlic roasted potatoes', price: '€31', category: 'Food', image: 'https://images.unsplash.com/photo-1467003909585-2f8a72700288' },
];
...
async function populate() {
    ...
    // Clean up IDs or use upsert with name as key if possible
    const { error } = await supabase.from('menu_items').upsert(allItems, { onConflict: 'name' });

    if (error) {
        console.error('Error populating database:', error);
    } else {
        console.log('Successfully updated menu with real items from ravinteliolkkari.fi');
    }
}
